...
const handler = (event, context, cb) => {
  ...
  try {
    ...
    if (event && event.method && event.method === 'GET') {
      ...
      if (event && !event.body) {
        logger.error("POST request body is empty: " );
      ...
      exportable.validateQueryInput(event)
        ...
        .then((res) => {
          logger.debug("POST response: " + JSON.stringify(res));
      ...
      exportable.validateQueryInput(event)
        ...
        .then((res) => {
          logger.debug("DELETE response: " + JSON.stringify(res));
          return cb(null, responseObj(res, event.body));
        }).catch((error) => {
...
const getConfiguration = () => {
  return new Promise((resolve, reject) => {
    crud.get(function (err, data) {
      if (err) {
        logger.error("getConfiguration: " + JSON.stringify(err));
        return reject(err);
      } else {
        logger.debug("getConfiguration: " + JSON.stringify(data));
        return resolve(data);
      }
    });
  });
...
const addConfiguration = (configs, event) => {
  return new Promise((resolve, reject) => {
    ...
    if (isEmpty(event.query)) {
      ...
      logger.debug("addConfiguration: editJson: " + JSON.stringify(res));
      if (!res.isError) {
        new_config = res.data
      } else {
    ...
    crud.post(new_config, function (err, data) {
      if (err) {
        logger.error("addConfiguration: err: " + JSON.stringify(err));
        return reject(err);
      } else {
        logger.debug("addConfiguration: res: " + JSON.stringify(data));
        return resolve(data);
      }
    });
  });
...
const deleteConfiguration = (configs, event) => {
  return new Promise((resolve, reject) => {
    ...
    if (isEmpty(event.query)) {
      ...
      logger.debug("deleteConfiguration: editJson: " + JSON.stringify(res));
      if (!res.isError) {
        new_config = res.data
      } else {
        return reject(res.error);
    ...
    crud.post(new_config, function (err, data) {
      if (err) {
        logger.error("deleteConfiguration: err: " + JSON.stringify(err));
        return reject(err);
      } else {
        logger.debug("deleteConfiguration: res: " + JSON.stringify(data));
        return resolve(data);
      }
    });
  });
